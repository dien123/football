...
const ResultsPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-[#111] text-slate-100 pb-24">
      ...
      <div className="max-w-4xl mx-auto px-6 space-y-12">
        ...
        <section className="animate-fade-in">
          <div className="flex items-center gap-2 mb-4 text-[10px] font-black uppercase tracking-[0.2em] text-emerald-400">
            ...
            <div className="divide-y divide-white/5">
              {results.betResults.length > 0 ? results.betResults.map((bet: any) => (
                <div key={bet.id} className="grid grid-cols-12 gap-4 px-6 py-4 text-xs items-center hover:bg-white/5 transition-colors">
                  <span className="col-span-2 font-black text-[11px] text-slate-400 truncate">{bet.user_name}</span>
                  <div className="col-span-3 flex flex-col gap-1">
                    ...
                    {selectedMatchId === 'all' && (
                      <span className="text-[8px] text-slate-600 truncate font-black uppercase tracking-tighter">{bet.matchName}</span>
                    )}
                  </div>
                  <div className="col-span-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`font-black ${bet.winningTeam === 'Hòa' ? 'text-slate-500 border-b border-white/5 pb-0.5' : 'text-emerald-400'}`}>
                        {bet.winningTeam}
                        <span className="text-slate-500 ml-2 font-mono text-[12px] font-medium">({bet.matchScore})</span>
                      </span>
                    </div>
                  </div>
        ...
        {leaderboard.length > 0 && (
          <section className="animate-fade-in delay-200">
            <div className="flex items-center gap-2 mb-6 text-xs font-black uppercase tracking-widest text-amber-400">
